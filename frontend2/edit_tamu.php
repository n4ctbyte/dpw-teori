    ...
    <div class="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md border border-terra/20">
        ...
        <form action="" method="POST" class="flex flex-col gap-4">
            <div>
                ...
                <input type="text" name="nama" value="<?php echo htmlspecialchars($data['nama']); ?>" required class="w-full bg-cream border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-terra">
            </div>
            ...
            <div>
                ...
                <select name="kehadiran" required class="w-full bg-cream border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-terra">
                    <option value="hadir" <?php if($data['kehadiran'] == 'hadir') echo 'selected'; ?>>Hadir</option>
                    <option value="tidak" <?php if($data['kehadiran'] == 'tidak') echo 'selected'; ?>>Tidak Hadir</option>
                    <option value="ragu" <?php if($data['kehadiran'] == 'ragu') echo 'selected'; ?>>Ragu-ragu</option>
                    <option value="belum" <?php if($data['kehadiran'] == 'belum') echo 'selected'; ?>>Belum Respon</option>
                </select>
            </div>

            <div>
                <label class="block text-sm font-medium mb-1">Pesan / Ucapan</label>
                <textarea name="pesan" rows="3" class="w-full bg-cream border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-terra"><?php echo htmlspecialchars($data['pesan']); ?></textarea>
            </div>

            <div class="flex gap-3 mt-4">
                <a href="dashboard.php" class="w-1/2 text-center bg-gray-200 text-gray-700 py-2 rounded-lg font-medium hover:bg-gray-300 transition">Batal</a>
                <button type="submit" name="update" class="w-1/2 bg-terra text-white py-2 rounded-lg font-medium hover:bg-terra2 transition">Update</button>
            </div>
        </form>
    </div>
</body>
